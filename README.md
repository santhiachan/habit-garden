...
整个应用是**一个 HTML 文件**，没有构建工具、没有框架依赖，打开即用。不登录也能正常使用（数据保存在本地），登录后自动通过 Firebase 同步到云端，支持跨设备。
...
打开盲盒获得一颗随机种子 —— 现在共 **11 种植物** 随机掉落：

> 毛豆 · 玫瑰 · 番茄 · 仙人掌 · 龟背竹 · 风信子 · 含羞草 · 绣球花 · 蓝莓 · 橘子 · 茄子

你的植物会随经验值成长，经历 **6 个阶段**：

> 小种子 → 萌芽期 → 小芽豆 → 小绿豆 → 胖胖豆 → 星辰豆

成熟后会开花或结果，植物的表情也会随你的打卡状态变化 —— 开心、平静或沮丧。摸一下它头还会摇一摇 🫧

每次升级都会有一段小庆祝动画：植物周围撒星星 ✨、跳一段可爱的舞蹈、从旧形态过渡到新形态，并出现一句温暖的祝贺语。每完成一个任务还会放一次小烟花 🎆。

### 📋 灵活的任务系统

- **每日任务**：每天重置，适合日常养成（读书、练琴、早睡……）
- **定期任务**（每周）：7 天内完成一次即可，适合一周一次的习惯
- **一次性任务**：做完即消失，适合阶段性挑战
- **截止日期**（可选）：定期任务过期自动隐藏；一次性任务过期会变淡红色提醒
- **半完成机制**：先「启动」拿一半奖励，降低开始的阻力；完成后再拿另一半
- **四级难度**：🌱小事 · 🌿一般 · 🌳挑战 · 🌲大事，奖励从 5xp 到 60xp
- **启动门槛标签**：易 / 中 / 难，首页默认把最容易开始的 3 个任务置顶
- **置顶 / 今日必做**：给关键任务加标记，让它们在首页更显眼
- **点击任务条** 即可重新编辑任务

### 🏅 五维能力值

每个任务归属一个维度，完成后累计对应属性。五个维度在首页都有专属的手绘小图标：

> ✈ 事业 Career · ♡ 魅力 Charm · 🍯 财富 Wealth · 📖 智慧 Wisdom · 🌱 健康 Health

### 🎁 许愿商店

用打卡赚的 ⭐ 星星兑换奖励 —— 这些奖励是你平时「偷懒」会做或「犹豫太久」不敢做的事。兑换后请无罪恶感地享受。

- 支持自定义奖励和限购次数
- 图标从可爱表情池中挑选（每行 5 个，省空间又整齐）
- 每兑换一次都会下一阵小金币雨 🪙
...
五大维度各有独立卡池，传说卡极为稀有。已解锁的卡片会自动置顶显示，让你的收藏进度一眼可见。
...
- **心情统计**：按周/月/全部查看心情趋势
- **睡眠计划**：每晚记录打算几点睡
- **记录页图表**：任务完成曲线 + 睡眠频率玫瑰图（23 前 / 24 前 / 1 点前 / 更晚）

### 🫂 低落时的安慰

当你点击「今天有点不行」，不会得到说教 —— 只有一句温柔的话和一个拥抱按钮，还可以坐下休息 10 分钟。

---

## ☁️ 数据同步

- **未登录也能用**：所有数据自动保存在浏览器本地（localStorage），刷新不会丢
- **登录后云同步**：Google 账号或邮箱密码登录，数据通过 Firebase Authentication + Firestore 实时同步到云端
- **跨设备**：登录同一账号，手机和电脑随时接着玩
- **同步状态指示**：🟢 正常 · 🟡 同步中 · 🔴 出错
- **离线回退**：断网时继续用 localStorage，联网后静默同步
- **加载最新版**：设置里有「🔄 加载更新」按钮，一键拉取缓存外的最新前端代码
